...
if ($pdo instanceof PDO && $cnpjSess) {
    try {
        ...
        if (!$empresaRow) {
            ...
            $falt = [];
            foreach ($obrig as $k) {
                if (trim((string)($empresaRow[$k] ?? '')) === '') $falt[] = $k;
            }
            if ($falt) {
                $empresaPendente = true;
                $msgCompletar = 'Algumas informações da empresa estão faltando.';
            }
        }
    } catch (Throwable $e) {
    }
            ...
            <nav class="nav navbar navbar-expand-lg navbar-light iq-navbar">
                <div class="container-fluid navbar-inner">
                    ...
                    <div class="sidebar-toggle" data-toggle="sidebar" data-active="true">
                        <i class="icon"><svg width="20" class="icon-20" viewBox="0 0 24 24">
        ...
        (function() {
            ...
            const options = {
                ...
                chart: {
                    ...
                    fontFamily: 'inherit',
                    // força redesenho quando o pai/redimensiona
                ...
                // cores explícitas para garantir visibilidade em qualquer tema
                colors: ['#3b82f6'],
                stroke: {
                    curve: 'smooth',
                    width: 3,
            ...
            function renderWhenVisible() {
                if (chart) return; // já renderizou
                // aguarda o card ganhar tamanho real
                if (!hasSize(el)) return;
                chart = new ApexCharts(el, options);
                chart.render();
                // pequeno “nudge” para garantir cálculo final das dimensões
                setTimeout(() => window.dispatchEvent(new Event('resize')), 0);
            }

            // 1) Renderiza assim que ficar visível (IntersectionObserver)
            ...
            // 2) Fallback: tenta após o carregamento e depois de um tick
            window.addEventListener('load', () => {
                setTimeout(renderWhenVisible, 0);
                setTimeout(renderWhenVisible, 300);
            });

            // 3) Se o pai redimensionar (ex.: abrir/fechar sidebar), força redraw
            window.addEventListener('resize', () => {
                if (chart) chart.updateOptions({}); // força recalcular layout
            });

            // 4) Se o tema mudar (classe .dark no <html>), atualiza as cores
            ...
            // 5) Algumas UIs disparam eventos custom na abertura do sidebar/menu
            document.querySelectorAll('[data-toggle="sidebar"]').forEach(btn => {
                btn.addEventListener('click', () => setTimeout(() => {
                    if (chart) chart.updateOptions({});
                    else renderWhenVisible();
                }, 250));
            });
        })();
    </script>
