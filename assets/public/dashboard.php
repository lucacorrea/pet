...
if ($pdo instanceof PDO && $cnpjSess) {
  try {
    ...
    $vendasQtde=(int)$v['qtde']; $faturamento30d=(float)$v['total'];
  } catch (Throwable $e) {}
  try {
    $st=$pdo->prepare("SELECT COUNT(*) FROM produtos_peca WHERE empresa_cnpj=:c AND ativo=1");
    $st->execute([':c'=>$cnpjSess]); $itensEstoque=(int)$st->fetchColumn();
  } catch (Throwable $e) {}
}
...
if ($pdo instanceof PDO && $cnpjSess) {
  try {
    if ($range === 'week' || $range === 'month') {
      ...
      // labels d/m do mais antigo ao atual
      $labels=[]; $mapIndex=[];
      $start = (new DateTime('today'))->modify('-'.($dias-1).' days');
      for ($i=0;$i<$dias;$i++){
        $d=(clone $start)->modify("+$i days");
        $key=$d->format('Y-m-d'); $labels[]=$d->format('d/m'); $mapIndex[$key]=$i;
      }
      ...
      $st=$pdo->prepare("
        ...
        FROM vendas_peca
        WHERE empresa_cnpj=:c AND status='fechada'
          AND criado_em >= DATE_SUB(CURDATE(), INTERVAL :dias-1 DAY)
        GROUP BY DATE(criado_em)
        ORDER BY DATE(criado_em)
      ");
      $st->bindValue(':c',$cnpjSess);
      $st->bindValue(':dias',$dias,PDO::PARAM_INT);
      $st->execute();
      foreach($st->fetchAll(PDO::FETCH_ASSOC) as $r){
        $k=(string)$r['dia']; if(isset($mapIndex[$k])) $vals[$mapIndex[$k]]=(float)$r['total'];
      }
      ...
      for ($i=$meses-1; $i>=0; $i--){
        ...
        $key=$p->format('Y-m');        // chave p/ map
        $labels[]=$p->format('m/Y');   // label
        $mapIndex[$key]=count($labels)-1;
      }
      ...
      $st=$pdo->prepare("
        ...
        FROM vendas_peca
        WHERE empresa_cnpj=:c AND status='fechada'
          AND criado_em >= DATE_SUB(CURDATE(), INTERVAL :m-1 MONTH)
        GROUP BY YEAR(criado_em), MONTH(criado_em)
        ORDER BY YEAR(criado_em), MONTH(criado_em)
      ");
      $st->bindValue(':c',$cnpjSess);
      $st->bindValue(':m',$meses,PDO::PARAM_INT);
      $st->execute();
      foreach($st->fetchAll(PDO::FETCH_ASSOC) as $r){
        $k=(string)$r['ym']; if(isset($mapIndex[$k])) $vals[$mapIndex[$k]]=(float)$r['total'];
      }
      $chartLabels=$labels; $chartSeries=$vals;
    }
  } catch (Throwable $e) {}
...
<body>
  ...
  <main class="main-content">
    <?php if ($empresaPendente): ?>
      ...
      <script>document.addEventListener('DOMContentLoaded',()=>{var el=document.getElementById('modalEmpresaIncompleta');if(el&&bootstrap?.Modal){new bootstrap.Modal(el,{backdrop:'static',keyboard:false}).show();}});</script>
    <?php endif; ?>
  ...
  <!-- Render gráfico com cores forçadas (linha visível) -->
  <script>
    (function(){
      ...
      const options = {
        ...
        // Força cores para garantir linha visível em qualquer tema
        colors: ['#3b82f6'],
