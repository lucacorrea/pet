...
$vendasQtde = 0; $faturamento30d = 0.0; $itensEstoque = 0; $despesas30d = 0.0;
if ($pdo instanceof PDO && $cnpjSess) {
  try {
    $st = $pdo->prepare("
...
$limitaPct = static function (float $v, float $m): int { if ($m<=0) return 0; $p=(int)round(($v/$m)*100); return max(0,min(100,$p)); };
...
if ($pdo instanceof PDO && $cnpjSess) {
  try {
    if ($range === 'week' || $range === 'month') {
      ...
      // datas de início/fim (fim exclusivo)
      $ini = (new DateTime('today'))->modify('-'.($dias-1).' days')->setTime(0,0,0);
      $fim = (new DateTime('tomorrow'))->setTime(0,0,0);
      ...
      for ($i=0; $i<$dias; $i++) {
        ...
        $labels[] = $d->format('d/m');
        $mapIndex[$d->format('Y-m-d')] = $i;
      }
      $vals = array_fill(0, $dias, 0.0);

      $st = $pdo->prepare("
        SELECT DATE(criado_em) dia, COALESCE(SUM(total_liquido),0) total
        FROM vendas_peca
        WHERE empresa_cnpj=:c AND status='fechada'
      ...
      // início = primeiro dia do mês mais antigo; fim = 1º dia do mês seguinte ao atual
      $first = new DateTime('first day of this month');
      $ini   = (clone $first)->modify('-'.($meses-1).' months')->setTime(0,0,0);
      $fim   = (clone $first)->modify('+1 month')->setTime(0,0,0);

      $labels = []; $mapIndex = []; $vals = array_fill(0, $meses, 0.0);
      for ($i=$meses-1; $i>=0; $i--) {
        $p = (clone $first)->modify("-{$i} months");
        $labels[] = $p->format('m/Y');          // label
        $mapIndex[$p->format('Y-m')] = count($labels)-1; // chave
      }

      $st = $pdo->prepare("
        SELECT DATE_FORMAT(criado_em,'%Y-%m') ym, COALESCE(SUM(total_liquido),0) total
        FROM vendas_peca
        WHERE empresa_cnpj=:c AND status='fechada'
          AND criado_em >= :ini AND criado_em < :fim
...
if ($pdo instanceof PDO && $cnpjSess) {
  try {
    ...
    if(!$empresaRow){ $empresaPendente=true; $msgCompletar='Sua empresa ainda não está cadastrada.'; }
    else {
      $falt=[]; foreach($obrig as $k){ if(trim((string)($empresaRow[$k]??''))==='') $falt[]=$k; }
      if($falt){ $empresaPendente=true; $msgCompletar='Algumas informações da empresa estão faltando.'; }
    }
  } catch(Throwable $e){}
}
?>
...
<main class="main-content">
  <?php if ($empresaPendente): ?>
    ...
    <script>
      document.addEventListener('DOMContentLoaded',()=>{const el=document.getElementById('modalEmpresaIncompleta'); if(el&&bootstrap?.Modal){new bootstrap.Modal(el,{backdrop:'static',keyboard:false}).show();}});
    </script>
  <?php endif; ?>
    ...
    <nav class="nav navbar navbar-expand-lg navbar-light iq-navbar">
      <div class="container-fluid navbar-inner">
        ...
        <div class="sidebar-toggle" data-toggle="sidebar" data-active="true">
          <i class="icon"><svg width="20" class="icon-20" viewBox="0 0 24 24"><path fill="currentColor" d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z"/></svg></i>
        </div>
      ...
      <div class="container-fluid iq-container">
        <div class="row"><div class="col-md-12">
          ...
          <p><?=htmlspecialchars($fraseHeader,ENT_QUOTES,'UTF-8')?> Empresa:
            <strong><?=htmlspecialchars($empresaNome,ENT_QUOTES,'UTF-8')?></strong></p>
        </div></div>
      </div>
          ...
          <div class="overflow-hidden d-slider1">
            <ul class="p-0 m-0 mb-2 swiper-wrapper list-inline" style="gap:6px;">
              <li class="swiper-slide card card-slide col-lg-3" data-aos="fade-up" data-aos-delay="700">
                <div class="card-body"><div class="progress-widget">
                  <div id="circle-progress-01" class="text-center circle-progress-01 circle-progress circle-progress-primary"
                       data-min-value="0" data-max-value="100" data-value="<?=$vendasPct?>" data-type="percent"></div>
                  <div class="progress-detail"><p class="mb-2">Vendas</p><h4 class="counter"><?=number_format($vendasQtde,0,',','.')?></h4></div>
                </div></div>
              </li>
              <li class="swiper-slide card card-slide col-lg-3" data-aos="fade-up" data-aos-delay="800">
                <div class="card-body"><div class="progress-widget">
                  <div id="circle-progress-02" class="text-center circle-progress-01 circle-progress circle-progress-info"
                       data-min-value="0" data-max-value="100" data-value="<?=$estoquePct?>" data-type="percent"></div>
                  <div class="progress-detail"><p class="mb-2">Itens em Estoque</p><h4 class="counter"><?=number_format($itensEstoque,0,',','.')?></h4></div>
                </div></div>
              </li>
              <li class="swiper-slide card card-slide col-lg-3" data-aos="fade-up" data-aos-delay="900">
                <div class="card-body"><div class="progress-widget">
                  <div id="circle-progress-03" class="text-center circle-progress-01 circle-progress circle-progress-primary"
                       data-min-value="0" data-max-value="100" data-value="<?=$faturamentoPct?>" data-type="percent"></div>
                  <div class="progress-detail"><p class="mb-2">Faturamento</p><h4 class="counter">R$ <?=number_format($faturamento30d,2,',','.')?></h4></div>
                </div></div>
              </li>
              <li class="swiper-slide card card-slide col-lg-3 px-3" data-aos="fade-up" data-aos-delay="1100">
                <div class="card-body"><div class="progress-widget">
                  <div id="circle-progress-04" class="text-center circle-progress-01 circle-progress circle-progress-primary"
                       data-min-value="0" data-max-value="100" data-value="<?=$despesasPct?>" data-type="percent"></div>
                  <div class="progress-detail"><p class="mb-2">Despesas</p><h4 class="counter">R$ <?=number_format($despesas30d,2,',','.')?></h4></div>
                </div></div>
              </li>
            </ul>
          </div><!-- /slider -->
    ...
    <div class="col-md-12 col-lg-12">
      <div class="row"><div class="col-md-12">
        <div class="card" data-aos="fade-up" data-aos-delay="800">
          ...
          <div class="card-body"><div id="d-main" class="d-main"></div></div>
        </div>
      </div></div>
    </div>
  </div><!-- /container -->
...
<!-- Render do gráfico: renderiza só quando o container tem tamanho e faz redraw ao redimensionar -->
<script>
(function(){
  ...
  const options = {
    ...
    yaxis: {
      min: 0,
      labels: { formatter: v => 'R$ ' + Number(v||0).toLocaleString('pt-BR',{ maximumFractionDigits:0 }) }
    },
    colors: ['#3b82f6'],
  ...
  const io = new IntersectionObserver((ents)=>{
    ents.forEach(e=>{ if(e.isIntersecting){ renderWhenVisible(); io.disconnect(); }});
  },{threshold:0.1});
  io.observe(el);

  window.addEventListener('load', ()=>{ setTimeout(renderWhenVisible,0); setTimeout(renderWhenVisible,300); });
  window.addEventListener('resize', ()=>{ if(chart) chart.updateOptions({}); });

  const mo = new MutationObserver(()=>{ if(chart){ chart.updateOptions({ theme:{mode:isDark()?'dark':'light'}, markers:{ strokeColors:isDark()?'#0f172a':'#ffffff' } }, false, true); }});
  mo.observe(document.documentElement,{attributes:true, attributeFilter:['class']});

  document.querySelectorAll('[data-toggle="sidebar"]').forEach(btn=>{
    btn.addEventListener('click', ()=> setTimeout(()=>{ if(chart) chart.updateOptions({}); else renderWhenVisible(); }, 250));
  });
})();
</script>
</body>
