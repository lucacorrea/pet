...
/* ===== Tema ===== */
:root{
  --bg:#0b1130; --brand:#0d2f53; --brand2:#123b6a;
  --panel:#173e6d; --panel2:#1d4f86; --border:#2d5a93;
  --text:#eaf2ff; --muted:#c6d6f6; --ticket-edge:#ced7e6;
...
.nfce-input .inp{
  ...
  background:#f8fbff; color:#0e1b2a; border:2px dashed #7fb2ff; border-radius:12px;
  font-weight:700; letter-spacing:.02rem; box-shadow:inset 0 1px 0 rgba(255,255,255,.75);
}
.nfce-input .inp::placeholder{color:#7a8aa6}
.nfce-input .inp:focus{outline:none; border-color:#a7c6ff; box-shadow:0 0 0 4px rgba(127,178,255,.22)}
.nfce-input .inp-icon{
...
/* Sugestões */
#sug{ top:62px !important; display:none; max-height:300px; overflow:auto; border:2px dashed #cdd9f3 !important; z-index:1000 }
#sug .sug-item{ cursor:pointer; border-bottom:1px dotted #e3eaf8 }
...
/* Inputs base */
.inp{background:#fff;color:#0e1b2a;border:2px solid #7fb2ff;border-radius:10px;height:50px;padding:0 10px;font-weight:600;width:100%}
...
if(temCaixa){
  ...
  document.addEventListener('keydown',(e)=>{
    if(e.key==='F2'){e.preventDefault(); qtd.select();}
    if(e.key==='F4'){e.preventDefault(); const b=document.getElementById('btn-finalizar'); if(!b.disabled) (document.getElementById('form-venda').requestSubmit?document.getElementById('form-venda').requestSubmit(b):b.click()); }
  });
