    ...
    <style>
        ...
        * {
        ...
        #tbl thead th {
            ...
            z-index: 1;
            background: #0f2f59;
            border-bottom: 1px solid #2b5288;
            font-size: .75rem;
            letter-spacing: .06rem;
            text-transform: uppercase;
        }
        ...
        .table>:not(caption)>*>* {
            border-bottom: 1px dotted #2b5288
        }
        ...
        #tbl .form-control-sm:focus {
            border-color: #7fb2ff;
            box-shadow: 0 0 0 .2rem rgba(127, 178, 255, .15);
        }
        ...
        .totalzao {
            background: linear-gradient(180deg, #215a9d33, #1a4d8b22);
        ...
        if (temCaixa) {
            ...
            el('#tbl').addEventListener('input', (e) => {
                if (e.target.classList.contains('q')) {
                    ...
                    itens[i].qtd = Math.max(parseFloat(e.target.value || '0'), 0);
                    recalc();
                }
                if (e.target.classList.contains('u')) {
                    const i = +e.target.dataset.i;
                    itens[i].unit = Math.max(parseFloat(e.target.value || '0'), 0);
                    recalc();
                }
            });
            el('#tbl').addEventListener('click', (e) => {
                if (e.target.closest('.d')) {
                    const i = +e.target.closest('.d').dataset.i;
                    itens.splice(i, 1);
                    recalc();
                }
            });
