...
<head>
  ...
  <style>
    /* ===== Resetzinhos de espaçamento pra layout consistente ===== */
    .iq-navbar-header { height: auto !important; margin-bottom: 1.25rem !important; }
    .iq-header-img { display:none; } /* tira a imagem decorativa que estourava o layout */
    ...
    .pdv-visor .linha1 { font-size: 0.95rem; opacity: .85 }
    .pdv-visor .linha2 { font-size: 2.1rem; font-weight: 700; letter-spacing: .5px }
    ...
    .pdv-suggest {
      ...
      max-height: 320px; overflow: auto; z-index: 2000; display: none;
      border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;
      box-shadow: 0 12px 24px rgba(0,0,0,.08);
    }
    .pdv-suggest .item { padding: .6rem .75rem; cursor: pointer; }
    ...
    .itens-table thead th { font-weight: 600; white-space: nowrap; }
    ...
    /* Inputs grandes alinhados */
    .form-control-lg { height: 48px; }

    /* Botões */
    .kbtn { min-width: 90px; }
    .kbd {
      background: #0b1220; color: #cbd5e1; padding: .15rem .45rem;
      border-radius: .35rem; font-size: .85rem; border: 1px solid #1f2937;
    }
    .pay-btn { height: 52px; font-size: 1.02rem; }
    ...
    /* Sticky da coluna direita pra desktop */
        ...
        <div class="row align-items-start g-2">
          <div class="col-md-8">
            <div class="d-flex flex-column gap-1">
              <h1 class="mb-0">Venda Rápida</h1>
              <p class="mb-2">Fluxo de PDV para balcão — leitor de código, busca por nome/SKU/EAN, e finalização rápida.</p>

              <?php if (!$caixaAberto): ?>
                <div class="alert d-inline-flex align-items-center gap-2 py-2 px-3" style="font-weight:700; background:#ffecec; color:#b42318; border:1px solid #f8c9c6; border-radius:10px;">
                  <i class="bi bi-exclamation-triangle"></i>
                  <span>Nenhum caixa aberto no momento.</span>
                  <a href="../../caixa/pages/caixaAbrir.php" class="alert-link ms-1">Clique aqui</a> para abrir ou entrar em um caixa.
                </div>
              <?php else: ?>
                <div class="d-inline-flex align-items-center gap-2 px-3 py-2 rounded" style="background:#eef8ff; color:#0b4a74; border:1px solid #cfe8ff;">
            ...
            <div class="card-body">
              <div class="row g-2 align-items-end">
                <div class="col-lg-7">
                  ...
                  <div class="pdv-busca">
                    <input type="text" class="form-control form-control-lg" id="inp-busca" placeholder="Digite nome, SKU ou EAN" autocomplete="off" <?= $caixaAberto ? '' : 'disabled' ?>>
                ...
                <div class="col-12 d-flex flex-wrap gap-2 mt-2">
                  <button type="button" class="btn btn-primary kbtn" id="btn-add" <?= $caixaAberto ? '' : 'disabled' ?>>
                    <i class="bi bi-plus-lg"></i> Adicionar <span class="small ms-1 kbd">Enter</span>
                  </button>
                  <button type="button" class="btn btn-outline-secondary kbtn" id="btn-qtd" <?= $caixaAberto ? '' : 'disabled' ?>>
                    <i class="bi bi-123"></i> Qtd <span class="small ms-1 kbd">F2</span>
                  </button>
                  <button type="button" class="btn btn-outline-secondary kbtn" id="btn-desc" <?= $caixaAberto ? '' : 'disabled' ?>>
                    <i class="bi bi-percent"></i> Desconto
                  </button>
                  <button type="button" class="btn btn-outline-danger kbtn" id="btn-clear" <?= $caixaAberto ? '' : 'disabled' ?>>
...
<script>
  ...
  const fmt3 = v => (Number(v || 0)).toLocaleString('pt-BR', { minimumFractionDigits: 3, maximumFractionDigits: 3 });
  ...
  function validateFinalizeButton() {
    ...
    if (!temCaixa) { btn.disabled = true; return; }
