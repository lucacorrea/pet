...
if ($empresaCnpj) {
    ...
    ");
        ...
        if ($row = $stE->fetch(PDO::FETCH_ASSOC)) {
            foreach ($empresa as $k => $v) if (array_key_exists($k, $row) && $row[$k] !== null) $empresa[$k] = (string)$row[$k];
        }
    } catch (Throwable $e) {
    }
}
...
if ($vendaId > 0 && $empresaCnpj) {
    try {
        $st = $pdo->prepare("
      SELECT id, empresa_cnpj, vendedor_cpf, total_bruto, desconto, total_liquido, forma_pagamento,
...
$pgBrutos = (array)($_GET['pg'] ?? []); // ex.: pg[]=pix:10.00&pg[]=debito:5.50
$pagamentos = [];
$totalPg = 0;
...
<head>
    ...
    <style>
        ...
        /* IMPRESSÃO – alinhado à ESQUERDA (DANFE NFC-e) */
        @page {
    ...
    <div class="wrap">
        <?php if (!$venda): ?>
            <div class="cupom center">
                ...
                <div class="muted subtle">Verifique o código e tente novamente.</div>
                <div class="linha"></div>
        ...
        <?php else: ?>
            <div class="cupom">
                ...
                <div class="center" style="margin-bottom:6px">
                    ...
                    <div class="subtle">CNPJ: <?= h(fmt_doc($empresa['cnpj'])) ?></div>
                    <?php if ($empresa['endereco']): ?>
                        <div class="muted subtle"><?= h($empresa['endereco']) ?></div>
                    <?php endif; ?>
                    <div class="muted subtle">
                        <?= h(trim(($empresa['cidade'] ?: '') . ($empresa['estado'] ? ' / ' . $empresa['estado'] : '') . ($empresa['cep'] ? ' — CEP: ' . $empresa['cep'] : ''))) ?>
                    </div>
                    <?php if ($empresa['telefone'] || $empresa['email']): ?>
                        <div class="muted subtle">
                            <?= $empresa['telefone'] ? 'Fone: ' . h($empresa['telefone']) : '' ?>
                            <?= ($empresa['telefone'] && $empresa['email']) ? ' • ' : '' ?>
                            <?= $empresa['email'] ? 'E-mail: ' . h($empresa['email']) : '' ?>
                        </div>
                    <?php endif; ?>
                </div>

                <div class="linha"></div>
                <div class="center"><span class="pill mono">NFC-e não permite aproveitamento de crédito de ICMS</span></div>
                <div class="linha"></div>

                <!-- Tabela itens no estilo do print -->
                <table>
                    <thead>
                        <tr>
                            <th class="cod">Cód</th>
                ...
                <!-- Totais -->
                <div class="totais">
                    <div class="row"><span class="muted">QTD TOTAL DE ITENS</span><span class="mono"><?= nf($qtdTotal, 3) ?></span></div>
                    <div class="row"><span class="muted">SUBTOTAL</span><span class="mono"><?= moneyR($subtotal) ?></span></div>
                    <div class="row"><span class="muted">DESCONTO</span><span class="mono"><?= moneyR($desconto) ?></span></div>
                    <div class="row grand"><span>TOTAL</span><span class="mono"><?= moneyR($valorTotal) ?></span></div>
                </div>
                ...
                <div class="subtle">
                    ...
                    <div class="mono">VALOR PAGO&nbsp;&nbsp;<?= moneyR($totalPg) ?></div>
                    <div class="mono">TROCO&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?= moneyR($troco) ?></div>
                </div>
                ...
                <?php if ($chave): ?>
                    <div class="linha"></div>
                    <div class="subtle muted">CHAVE DE ACESSO</div>
                    <?php
                ...
                <div class="subtle">
                    ...
                    <div class="mono"><?= $consLabel ?>: <?= h(fmt_doc($consDoc)) ?></div>
                </div>

                <?php if ($lei12741 !== null): ?>
                    <div class="linha"></div>
                    <div class="subtle">Tributos aproximados (Lei 12.741/2012): <strong class="mono"><?= moneyR($lei12741) ?></strong></div>
                <?php endif; ?>

                <?php if ($obs): ?>
                    <div class="linha"></div>
                    <div class="subtle"><strong>Observações:</strong><br><?= nl2br(h($obs)) ?></div>
                <?php endif; ?>
                ...
                <div class="linha"></div>
                <div class="muted subtle center">DANFE NFC-e — Documento Auxiliar. Válido somente com a NFC-e autorizada pela SEFAZ.</div>
            </div>
        <?php endif; ?>
    </div>
